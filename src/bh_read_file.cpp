...
#if defined(_WIN32) || defined(_WIN32_)
uint8_t* bh_read_file_to_buffer(const char *filename, uint32 *ret_size) {
    uint8_t *buffer;
    int file;
...
uint8_t* bh_read_file_to_buffer(const char *filename, uint32 *ret_size) {
    ...
    file_size = static_cast<uint32>(stat_buf.st_size);

    /* At lease alloc 1 byte to avoid malloc failed */
    buf_size = file_size > 0 ? file_size : 1;

    if (!(buffer = static_cast<uint8_t*>(BH_MALLOC(buf_size)))) {
        printf("Read file to buffer failed: alloc memory failed.\n");
        close(file);
        return NULL;
    }
    ...
    read_size = static_cast<uint32>(read(file, buffer, file_size));
    close(file);
